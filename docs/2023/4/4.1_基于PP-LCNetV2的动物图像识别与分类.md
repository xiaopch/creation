...
* 1. `_ssld` 表示使用 `SSLD 蒸馏`后的模型。关于 `SSLD蒸馏` 的内容，详情 [SSLD 蒸馏]。
...
#### 使用paddlex进行dataset划分

```
!pip install paddlex
!paddlex --split_dataset --format ImageNet --dataset_dir ./train_data --val_value 0.2
## 0.8的训练集，0.2的测试集。Train数量为： 14246、Eval 数量为： 3557
```

train_data目录生成`train_list.txt` 和 `val_list.txt` 分别为训练集和验证集的标签文件。labels.txt为分类目录文件。

**备注：**

* 关于 `train_list.txt`、`val_list.txt`的格式说明，可以参考[PaddleClas分类数据集格式说明] 。
...
with open('./train_data/labels.txt','r') as f:
    lines=f.readlines()
    print(lines)
f_list=open('./train_data/label_list.txt','w')    
print(len(lines))
...
```shell
!python3 -m paddle.distributed.launch   tools/train.py  -c PPLCNetV2_base.yaml  -o Arch.pretrained=False     -o Global.device=gpu
```
...
Global:
  checkpoints: null
  pretrained_model: 'output/PPLCNetV2_base/best_model'
  output_dir: ./output/
  device: gpu
  save_interval: 1
...
DataLoader:
  Train:
    dataset:
      ...
      cls_label_path: ./train_data/train_list.txt
      transform_ops:
        - DecodeImage:
            to_rgb: True
            channel_first: False
  ...
  Eval:
    dataset: 
      ...
      image_root: ./train_data/
      cls_label_path: ./train_data/val_list.txt
      transform_ops:
        - DecodeImage:
            to_rgb: True
            channel_first: False
...
Infer:
  ...
  PostProcess:
    name: Topk
    topk: 5
    class_id_map_file: train_data/label_list.txt
...
```bash
!python3 tools/eval.py     -c PPLCNetV2/PPLCNetV2_base.yaml     -o Global.pretrained_model=output/PPLCNetV2_base/best_model
```
...
```python
!python3 tools/infer.py     -c PPLCNetV2_base.yaml  -o Global.pretrained_model=output/PPLCNetV2_base/best_model
```
...
!python tools/export_model.py \
    ...
    -o Global.save_inference_dir=./deploy/models/PPLCNetV2_base
```
