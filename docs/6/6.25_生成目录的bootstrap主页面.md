...
exclude = set(['.git','.py','.cmd'])

"""	get Header """
def AddHead(html_list):
...
def AddBody(dirname,html_list):
    try:
        ...
        for file in ls:
            if file == 'index.html':
                continue
            dn_temp=os.path.join(dirname,file)
            if(os.path.isdir(dn_temp)):
                html_list.append('<li><a style="background-color:#dddddd;" href="./%s/">%s</a></li>\n' % (file,file))
            else:
...
def generate_index_links(dirname):
        ...
        AddHead(content_list)
        AddBody(dirname,content_list)
        AddFooter(content_list)
        print(dirname)
